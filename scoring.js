...
$(document).ready(function () {
  ...
  function get_achievement() {// 「ランク付与関数」
  ...
  function get_pass_or_failure() {// 「合否判定関数」60点以下
    let subject_points = [Number($('#national_language').val()),
    Number($('#english').val()),
    Number($('#mathematics').val()),
    Number($('#science').val()),
    ...
    let judge = "合格";
    for(let i=0; i<number; i++){// 入力したそれぞれの教科のうち、1つでも60点よりも低い点数があった場合、変数「judge」に"不合格"を再代入する処理を記述する。
      if (subject_points[i] < 60){// ヒント：配列の繰り返し処理について調べてみましょう。
        judge = "不合格";
        break;//繰り返し終了
      }
    }
